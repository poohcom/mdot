...
class SceneLoading extends Scene {
    ...
    private logo: createjs.Bitmap = null;
    private logos: Array<createjs.Bitmap> = [];
    ...
    constructor() {
        ...
        //1.BGM : 계속 루프로 배경음악을 돌려주세요 
        //배경음악은 현재 해주신것과 동일합니다.
        //2.Mouse over : 1_나무가있는 메인페이지에서 돌아가는 5개의 다각형과목 마우스오버시 나타나는 소리
        //2_각페이지의 X(엑스표시)에 마우스오버 할때 나타나는 소리(site map, exhibition페이지...등)
        //3_과목페이지에서 List버튼 마우스 오버 할때 나타나는 소리
        //4_과목페이지 각각의 썸네일 마우스 오버 할 나타나는 소리
        //3.Tree Mouse over: 1_메인페이지에서 나무부분에 마우스오버해서 위에서 아래로 움직일때 나타나는 크리스탈이 부딪히는 소리
        //2_STUDENTS 페이지플렉서스 마우스오버할때 나타나는 크리스탈 부딪히는 소리
        //4.Mouse click : 1_메인페이지에서 나무부분 밑에 다각형과목 클릭시 나타나는 소리
        //5.Page movement: 1_메인페이지에서 어바웃미디어(media design)로 이동할때 나타나는 소리
        //2_포트폴리오에서 메인페이지로 위로 올라갈때 나타나는 크리스탈이 부딪히는 소리
...
//        SceneManager.instance().queue.on("progress", handleProgress, this);
        SceneManager.instance().queue.loadManifest([
            ...
            //{ id: "e", src: "assets/intro/e.png" },
            //{ id: "n", src: "assets/intro/n.png" },
            //{ id: "o", src: "assets/intro/o.png" },
            //{ id: "t", src: "assets/intro/t.png" },
    ...
    public playScene(): void {
        this.count++;
        var n:number = ( Math.floor(this.count/3 ) )%29;
        for (var i: number = 0; i < this.logos.length; i++) {
        ...
        var progresPrecentage: number = Math.round(SceneManager.instance().queue.progress * 100);
