...
var ScenePortPoster = (function (_super) {
    ...
    function ScenePortPoster() {
        _super.call(this);
        this.thumbtext = ["고현경", "권은혜", "김민지", "김보경", "김선형", "김수려", "김수현", "김아란", "김연희", "김주연", "김지영", "김혜주", "나현아", "노효정", "도미래", "박경희", "박성현", "박정윤", "배하린", "송연아", "순별이", "신민아", "신효진", "양수련", "오남경", "오명선", "오연택", "윤새봄", "윤소담", "윤지혜", "이다정", "이민아", "이서현", "이소라", "이소영", "이예림", "이정민", "이주희", "임소리", "임주희", "정미애", "정민주", "정보경", "정주희", "정지선", "정채원", "정하나", "조다예", "조은아", "최새봄나", "하진아", "한소린", "허초미", "홍보미"];
        this.group = null;
        this.dx = 0;
        this.dr = 0;
    ...
    ScenePortPoster.prototype.init = function () {
        ...
        for (var i = 0; i < this.textButtonList.length; i++) {
            this.textButtonList[i].on("click", function () {
                var index = ScenePortPoster.instance().textButtonList.indexOf(this) + 1;
    ...
    ScenePortPoster.prototype.startScene = function () {
        ...
        SceneManager.instance().stage.on("stagemousemove", function (evt) {
            var w = window.innerWidth;
            var h = window.innerHeight;
            var dd = (evt.stageX / window.innerWidth - 0.5) * 10;
            ScenePortPoster.instance().dx = -dd * dd * dd;
        });
    };
    ScenePortPoster.prototype.drawing = function (event) {
    ...
    ScenePortPoster.prototype.makeButton = function () {
        ...
        for (var i = 0; i < this.buttonCount; i++) {
            var button = new PortCircleButton(this.getTex("students_work_button"), "assets/16th_web/poster/thumbnail/" + (i + 1) + ".png", 18, 53, this.thumbtext[i], null);
            button.index = i + 1;
            ...
            button.on("mouseover", function (event) {
                ScenePortPoster.instance().mouseOver(event.currentTarget.index);
            });
            button.on("mouseout", function (event) {
                ScenePortPoster.instance().mouseOut(event.currentTarget.index);
            });
            button.alpha = 0;
    ...
    ScenePortPoster.prototype.makeListTextButton = function () {
        var startY = 44;
        for (var i = 0; i < this.listText.length; i = i + 3) {
            var dy = 23;
    ...
    ScenePortPoster.prototype.drawLines = function () {
        this.count++;
    ...
    ScenePortPoster.prototype.drawLine2 = function (a1, a2, b1, b2, collide) {
        this.g.moveTo(0, 540);
        var r = 0;
        var c = Math.PI / (960 * 4);
        for (var i = 20; i < 6480 + 600; i = i + 10) {
            r = c * i;
    ...
    ScenePortPoster.prototype.drawSine = function (a1, a2, a3, b1, b2, b3, height, collide) {
        this.g.moveTo(0, 540);
        var r = 0;
        //var c: number = Math.PI / (960 * 4);
        var c = Math.PI / (6780 / 2);
        for (var i = 20; i < 6480 + 600; i = i + 10) {
            r = c * i;
    ...
    ScenePortPoster.prototype.fx2 = function (r, a1, a2, a3, b1, b2, b3) {
