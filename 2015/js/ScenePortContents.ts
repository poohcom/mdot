...
class ScenePortContents extends ScenePort {
    ...
    public init(): void {
        this.listText = [
            "Bluelink Infortainment \n& Smart Watch", "Booze", "E- SMART", "Kids Keeper", "Midi Kiosk", "Multi Talk", "My Fitness Partner", "서울시티투어버스 키오스크", "한글 박물관"
        ];
        ...
        for (var i: number = 0; i < this.textButtonList.length; i++) {
            this.textButtonList[i].on("click", function () {
                var index: number = ScenePortContents.instance().textButtonList.indexOf(this) + 1;
    ...
    private makeButton(): void {
        ...
        for (var i: number = 0; i < this.buttonCount; i++) {
            ...
            var button: PortCircleButton = new PortCircleButton(this.getTex("students_work_button"), "assets/16th_web/contents/thumbnail/" + (i + 1) + ".png", 18, 53,
                this.thumbtext[i][0],
                this.thumbtext[i][1]
                );
            button.index = i+1;
            ...
            button.on("mouseover", function (event: createjs.MouseEvent) {
                ScenePortContents.instance().mouseOver((<PortCircleButton>event.currentTarget).index);
            });

            button.on("mouseout", function (event: createjs.MouseEvent) {
                ScenePortContents.instance().mouseOut((<PortCircleButton>event.currentTarget).index);
            });
